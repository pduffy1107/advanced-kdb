...
upd:{[table;data]
    if[(table=`trade) and (any filterSym in/: flip data);						// If row contains "trade" schema and symbol in row...
        newLogHandle enlist (`upd;table;flip (flip data) where filterSym in/: flip data)];		// update new Log file with filtered data. 
    };
